...
function header(doc, page = 1) {
  ...
  doc.text("TRUE PEAK HOUSE LLP", pageWidth / 3, yPosition, {
    align: "center",
  });
...
function Container1(doc, formData) {
  ...
  return new Promise((resolve) => {
    img.onload = function () {
      ...
      doc.setFont("helvetica", "bold");
      doc.text(`${bank_infos.managerDesignation},`, margin + 5, yPosition + 12);
      doc.text(`${bank_infos.bankName},`, margin + 5, yPosition + 19);
      doc.text(`${bank_infos.branchLocation},`, margin + 5, yPosition + 26);
      ...
      let table = createTable(
        ...
        yPosition,
        null,
        "Type of Property – Under Construction Residential Land & Building"
      );
      ...
      yPos = table.addRow(
        "2",
        "Persons Accompanying / Contact No.",
      ...
      yPos = table.addRow(
        "a",
        "Name of Owner",
        "Mrs RIZWANA.A.PATEL & Mr ALTAF.D.PATEL"
      );
      ...
      yPos = table.addRow(
        "b",
        "Nearby Landmark/Google Map /\nIndependent access to the property",
        "Located near to R T Nagar Police Station on 80 ft Main Road, R T Nagar. It is about 09 kms from Bangalore City Bus Stand & Railway Station/ Google Map enclosed/ Yes it is Independently accessible.",
        25
      );
      ...
      yPos = table.addRow(
        ...
        "Approved by ADTP (East), BBMP"
      );
      yPos = table.addRow(
        "iii)",
        "Approval No / Date / Details",
        "BBMP/Ad.com/EST/0101/24-25"
      );
      yPos = table.addRow(
        "b",
        "Legal & Other Documents",
        "Photo Copies of Sale Deed dated 12/04/2001, Katha dated 30/03/2024, Tax paid receipt dated 03/04/2024 & Sanction Plan",
        16
      );
...
function Container3(doc, formData) {
  ...
  yPos = table.addRow(
    ...
    formData.variationJustification,
    30
  );
...
function Container4(doc, formData) {
  ...
  function addRow(label, description, amount, isBold = false) {
    ...
    doc.rect(
      leftMargin + colWidths.serial + colWidths.description,
      yPos,
      colWidths.amount,
      7
    );
    ...
    if (amount) {
      doc.text(
        amount,
        leftMargin + colWidths.serial + colWidths.description + 2,
        yPos + 5
      );
    }
  ...
  addRow(
    "4",
    "Bore well / Overhead Tank / Sump / Solar",
    formData.boreWellStatus
  );
...
function buildingValueTable(doc, formData) {
  ...
  addRow(
    "",
    "AS PER PLAN- For Present Stage Considered",
    "",
    "",
    "",
    "",
...
function Container5(doc) {
  ...
  function addAbstractRow(
    ...
    doc.rect(
      leftMargin + abstractColWidths.slNo,
      yPos,
      abstractColWidths.particulars,
      7
    );
    doc.rect(
      leftMargin + abstractColWidths.slNo + abstractColWidths.particulars,
      yPos,
      abstractColWidths.amount,
      7
    );
    ...
    if (amount) {
      doc.text(
        amount,
        leftMargin + abstractColWidths.slNo + abstractColWidths.particulars + 2,
        yPos + 5
      );
    }
...
function Container6(doc) {
  ...
  function addDeclarationRow(serial, content, isBold = false) {
    const height = Math.max(
      7,
      Math.ceil(doc.splitTextToSize(content, colWidths.content - 4).length * 7)
    );
  ...
  addDeclarationRow(
    "ii)",
    "The analysis and conclusion are limited by the reported assumptions and conditions."
  );
  addDeclarationRow(
    "iii)",
    "I have read the Handbook on policy, Standard and Procedures for real Estate valuation by banks and HFIs in India, 2011, issued by IBA and NHB, fully understood the provisions of the same and followed the provisions of the same to the best of my ability and this report is in conformity to the standards of Reporting enshrined in the above Handbook."
  );
  addDeclarationRow(
    "iv)",
    "I have no direct or indirect interest in the above property valued."
  );
  addDeclarationRow(
    "v)",
    "The Property was inspected by me on 03/12/2024 along with the Owner."
  );
  addDeclarationRow(
    "vi)",
    "I am a registered valuer under section 34AB of Wealth tax act, 1957, CAT-I/Reg.No.02/PCIT-I/CCIT/BNG-1/2023-24 for valuing property."
  );
  addDeclarationRow(
    "vii)",
    "The valuation made with reference to the photocopies given only irrespective of the legal opinion"
  );
  addDeclarationRow(
    "viii )",
    "The validity of the report is only for 3 months and is to be used purpose intended for and not for any other purpose"
  );
...
function Container7(doc) {
  ...
  addDetailsRow(
    "Address",
    "# 240/C, First floor, 3rd Block, Nagarabhavi 2nd stage, Bangalore-560072"
  );
  addDetailsRow(
    "Name of valuer association of which I am a\nbonafide member in good standing",
    "Indian Institute of Engineers",
    15
  );
  addDetailsRow(
    "Wealth tax Registration No",
    "CAT-I/Reg.No.02/PCIT-I/CCIT/BNG-1/2023-24"
  );
...
function createTable(doc, startY, title, subtitle = null) {
  ...
  function addAddressRow(number, description, addressData) {
    ...
    doc.rect(
      col3Start,
      yPosition + 3 * childRowHeight,
      lastRowCol1Width,
      childRowHeight
    );
    doc.rect(
      col3Start + lastRowCol1Width,
      yPosition + 3 * childRowHeight,
      col3Width - lastRowCol1Width,
      childRowHeight
    );
    ...
    doc.text(
      ...
      yPosition + childRowHeight + 5
    );
    doc.text(
      row2[2],
      col3Start + 2 * col3SplitWidth + 2,
      yPosition + childRowHeight + 5
    );
    doc.text(
      row2[3],
      col3Start + 3 * col3SplitWidth + 2,
      yPosition + childRowHeight + 5
    );
    ...
    doc.text(
      row4[1],
      col3Start + lastRowCol1Width + 2,
      yPosition + 3 * childRowHeight + 5
    );
